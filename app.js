(function () {
  ...
  function renderCardsPage(page) {
    ...
    return `
      <section class="page page-cards" id="${escapeAttr(page.id)}" data-page-section="${escapeAttr(page.id)}">
        <div class="cards-page-inner">${heading}${intro}${renderCards(page)}</div>
      </section>`;
  }
  ...
  function renderCanvasItem(item) {
    ...
    if (item.kind === "paper") {
      ...
      const links = item.links?.length ? `<ul class="link-list">${item.links.map(link =>
    ...
    if (item.kind === "photo") html = `<figure class="photo"><img src="${escapeAttr(item.src)}" alt="${escapeAttr(item.alt || "")}" /></figure>`;
    if (item.kind === "image") html = `<div class="image-only"><img src="${escapeAttr(item.src)}" alt="${escapeAttr(item.alt || "")}" /></div>`;
    if (item.kind === "button") html = `<div class="button-card"><a href="${escapeAttr(item.href)}" target="${externalTarget(item.href)}" rel="${externalRel(item.href)}">${escapeHtml(item.label)}</a></div>`;
